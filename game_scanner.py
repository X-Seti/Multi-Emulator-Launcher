"""
Game Scanner
Scans ROM directories, handles ZIP files, multi-disk games, and folder structures
"""
...
class GameScanner:
    def __init__(self, config, platforms):
        ...
        # Non-ROM file extensions to skip
    ...
    def scan_platform(self, platform_name):
        ...
        for item in platform_path.iterdir():
            ...
            # Handle folders (like Turrican-II-The-Final-Fight_Amstrad-CPC)
            elif item.is_dir():
    ...
    def _scan_folder(self, folder_path, platform_name, extensions):
        """Scan folder-based game (like multi-disk games in folders)"""
        rom_files = []
        disk_folders = []
        
        # Look for files and subfolders
        ...
        # Check for multi-disk structure
        disk_files = self._detect_multidisk(rom_files)
    ...
    def _create_game_entry(self, file_path, platform_name):
    ...
    def _is_valid_rom(self, filename, extensions):
    ...
    def _detect_multidisk(self, file_list):
        """Detect and order multi-disk games"""
        # Patterns for multi-disk detection
        patterns = [
            r'disk\s*(\d+)',
            r'disc\s*(\d+)',
            r'side\s*([ab])',
        ...
        for filename in file_list:
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    # Convert side letters to numbers
                    if disk_num.isalpha():
    ...
    def _group_multidisk_games(self, games):
        """Group games that are split across multiple files"""
        # Pattern to match game names with disk indicators
        ...
        for game in games:
            # Skip already processed multi-disk games
            if game.get('disk_count', 0) > 1:
                single_games.append(game)
                continue
            
            # Try to match disk pattern
        ...
        # Convert groups to multi-disk entries
        for base_name, disk_list in grouped.items():
            if len(disk_list) > 1:
                ...
                single_games.append({
                    ...
                    'type': 'multidisk',
                    'path': disk_list[0]['path'],  # First disk path
    ...
    def _clean_name(self, name):
        ...
        cleaned = re.sub(r'\[.*?\]', '', name)  # Remove [tags]
        cleaned = re.sub(r'\(.*?\)', '', cleaned)  # Remove (tags)
        cleaned = re.sub(r'[_-]+', ' ', cleaned)  # Replace _ and - with spaces
