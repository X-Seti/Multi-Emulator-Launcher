...
# X-Seti - December27 2025 - Multi-Emulator Launcher - Embedded Emulator Display

"""
Embedded Emulator Display Widget
Embeds external emulator windows into the display panel
Provides pop-out, fullscreen, window controls, and auto-resize
"""
...
# _force_resize_window
# _resize_embedded_window
# _setup_ui

class EmulatorEmbedWidget(QWidget): #vers 2
    """Widget that embeds external emulator windows with auto-resize support"""
    ...
    def __init__(self, parent=None, main_window=None, include_controls=True): #vers 3
        ...
        self.embed_retry_count = 0  # Track embed retry attempts
        self.max_embed_retries = 10  # Stop after 10 attempts
    ...
    def embed_window(self, process): #vers 2
        ...
        self.embed_retry_count = 0  # Reset retry counter
        
        # Wait a moment for window to appear
        QTimer.singleShot(500, self._attempt_embed)
        
    def _attempt_embed(self): #vers 3
        """Attempt to find and embed the emulator window"""
        if not self.emulator_process:
            return
        
        # Check retry limit
        if self.embed_retry_count >= self.max_embed_retries:
            print(f"⚠ Failed to embed window after {self.max_embed_retries} attempts")
            print("  Emulator is running but not embedded - use it in external window")
            return
        ...
        if window_id:
            try:
                ...
                if window:
                    ...
                    print(f"✓ Embedded window ID: {window_id} (attempt {self.embed_retry_count})")
                    
                    # Force resize the actual X11 window
                    QTimer.singleShot(200, lambda: self._force_resize_window(window_id))
            ...
            if self.embed_retry_count < self.max_embed_retries:
                # Only print every 3rd attempt to reduce spam
                if self.embed_retry_count % 3 == 0:
                    print(f"Could not find emulator window (attempt {self.embed_retry_count}/{self.max_embed_retries})")
                # Try again after a delay
                QTimer.singleShot(1000, self._attempt_embed)
            else:
                print(f"⚠ Could not find emulator window after {self.max_embed_retries} attempts")
    ...
    def _resize_embedded_window(self, window_id): #vers 1
        """Resize the embedded X11 window to fill container
        
        Args:
            window_id: X11 window ID
        """
        try:
            # Get embed_frame dimensions
            width = self.embed_frame.width()
            height = self.embed_frame.height()
            
            # Use xdotool to resize the window
            subprocess.run(
    ...
    def _force_resize_window(self, window_id): #vers 1
        """Aggressively force window to fill container using multiple methods
        ...
        try:
            ...
            print(f"Force resizing window {window_id} to {width}x{height}")
            
            # Method 1: Direct windowsize
            subprocess.run(
                ['xdotool', 'windowsize', str(window_id), str(width), str(height)],
                capture_output=True, timeout=1
            )
            
            # Method 2: Unmap and remap to force refresh
            subprocess.run(['xdotool', 'windowunmap', str(window_id)], capture_output=True, timeout=1)
            subprocess.run(['xdotool', 'windowmap', str(window_id)], capture_output=True, timeout=1)
            
            # Method 3: Set window size again after remap
            subprocess.run(
                ['xdotool', 'windowsize', str(window_id), str(width), str(height)],
                capture_output=True, timeout=1
            )
            
            # Method 4: Try to maximize if supported
            subprocess.run(
                ['xdotool', 'windowstate', '--add', 'MAXIMIZED_VERT', str(window_id)],
                capture_output=True, timeout=1
            )
            subprocess.run(
                ['xdotool', 'windowstate', '--add', 'MAXIMIZED_HORZ', str(window_id)],
                capture_output=True, timeout=1
            )
            
            print(f"✓ Force resize complete")
            
        except Exception as e:
            print(f"Error force resizing window: {e}")
